...
/* -- Export macros ------------------------------------------------------------ */
    ...
    #define ULTRAFAST_SECP256K1_API __declspec(dllexport)
  #else
    #define ULTRAFAST_SECP256K1_API __declspec(dllimport)
  #endif
#elif __GNUC__ >= 4
  #define ULTRAFAST_SECP256K1_API __attribute__((visibility("default")))
#else
  #define ULTRAFAST_SECP256K1_API
#endif
 ...
 *  Returns 0 on success. */
ULTRAFAST_SECP256K1_API int ultrafast_secp256k1_ec_pubkey_parse(
    const uint8_t* input,
    size_t input_len,
    uint8_t pubkey_out[33]);
 ...
 *  Returns 0 on success. */
ULTRAFAST_SECP256K1_API int ultrafast_secp256k1_ec_privkey_negate(uint8_t privkey[32]);

/** Add tweak to private key in-place: key = (key + tweak) mod n.
 *  Returns 0 on success. */
ULTRAFAST_SECP256K1_API int ultrafast_secp256k1_ec_privkey_tweak_add(
    uint8_t privkey[32],
    const uint8_t tweak[32]);

/** Multiply private key by tweak in-place: key = (key * tweak) mod n.
 *  Returns 0 on success. */
ULTRAFAST_SECP256K1_API int ultrafast_secp256k1_ec_privkey_tweak_mul(
    uint8_t privkey[32],
    const uint8_t tweak[32]);

/** Verify that a private key is valid (non-zero, < curve order).
 *  Returns 1 if valid, 0 if invalid. */
ULTRAFAST_SECP256K1_API int ultrafast_secp256k1_ec_seckey_verify(const uint8_t privkey[32]);
 ...
 *  Returns 0 on success. */
ULTRAFAST_SECP256K1_API int ultrafast_secp256k1_ecdsa_sign(
 ...
 *  Returns 1 if valid, 0 if invalid. */
ULTRAFAST_SECP256K1_API int ultrafast_secp256k1_ecdsa_verify(
    const uint8_t msg_hash[32],
 ...
 *  Returns 0 on success. */
ULTRAFAST_SECP256K1_API int ultrafast_secp256k1_ecdsa_signature_serialize_der(
    const uint8_t sig[64],
    uint8_t* der_out,
    size_t* der_len);
 ...
 *  Returns 0 on success. */
ULTRAFAST_SECP256K1_API int ultrafast_secp256k1_ecdsa_sign_recoverable(
    const uint8_t msg_hash[32],
    const uint8_t privkey[32],
    uint8_t sig_out[64],
    int* recid_out);
 ...
 *  Returns 0 on success. */
ULTRAFAST_SECP256K1_API int ultrafast_secp256k1_ecdsa_recover(
    const uint8_t msg_hash[32],
    const uint8_t sig[64],
    int recid,
    uint8_t pubkey_out[33]);
 ...
 *  Returns 0 on success. */
ULTRAFAST_SECP256K1_API int ultrafast_secp256k1_schnorr_sign(
 ...
 *  Returns 1 if valid, 0 if invalid. */
ULTRAFAST_SECP256K1_API int ultrafast_secp256k1_schnorr_verify(
    const uint8_t msg[32],
    const uint8_t sig[64],
 ...
 *  Returns 0 on success. */
ULTRAFAST_SECP256K1_API int ultrafast_secp256k1_schnorr_pubkey(
    const uint8_t privkey[32],
 ...
 *  Returns 0 on success. */
ULTRAFAST_SECP256K1_API int ultrafast_secp256k1_ecdh(
    const uint8_t privkey[32],
    const uint8_t pubkey[33],
 ...
 *  Returns 0 on success. */
ULTRAFAST_SECP256K1_API int ultrafast_secp256k1_ecdh_xonly(
    const uint8_t privkey[32],
    const uint8_t pubkey[33],
    uint8_t secret_out[32]);
 ...
 *  Returns 0 on success. */
ULTRAFAST_SECP256K1_API int ultrafast_secp256k1_ecdh_raw(
    const uint8_t privkey[32],
    const uint8_t pubkey[33],
    uint8_t secret_out[32]);
...
ULTRAFAST_SECP256K1_API void ultrafast_secp256k1_sha256(
    const uint8_t* data,
    size_t data_len,
...
ULTRAFAST_SECP256K1_API void ultrafast_secp256k1_hash160(
    const uint8_t* data,
    size_t data_len,
 ...
 *  digest_out: 32-byte output. */
ULTRAFAST_SECP256K1_API void ultrafast_secp256k1_tagged_hash(
    const char* tag,
    const uint8_t* data,
    size_t data_len,
    uint8_t digest_out[32]);
 ...
 *  Returns 0 on success. */
ULTRAFAST_SECP256K1_API int ultrafast_secp256k1_address_p2pkh(
    const uint8_t pubkey[33],
    int network,
    char* addr_out,
    size_t* addr_len);
 ...
 *  Returns 0 on success. */
ULTRAFAST_SECP256K1_API int ultrafast_secp256k1_address_p2wpkh(
    const uint8_t pubkey[33],
    int network,
    char* addr_out,
    size_t* addr_len);
 ...
 *  addr_out: buffer (min 62 + 1 bytes).
 *  Returns 0 on success. */
ULTRAFAST_SECP256K1_API int ultrafast_secp256k1_address_p2tr(
 ...
 *  Returns 0 on success. */
ULTRAFAST_SECP256K1_API int ultrafast_secp256k1_wif_encode(
    const uint8_t privkey[32],
    int compressed,
    int network,
    char* wif_out,
 ...
 *  Returns 0 on success. */
ULTRAFAST_SECP256K1_API int ultrafast_secp256k1_wif_decode(
    const char* wif,
 ...
 *  seed: 16-64 bytes.
 *  Returns 0 on success. */
ULTRAFAST_SECP256K1_API int ultrafast_secp256k1_bip32_master_key(
    const uint8_t* seed,
    size_t seed_len,
 ...
 *  Returns 0 on success. */
ULTRAFAST_SECP256K1_API int ultrafast_secp256k1_bip32_derive_child(
 ...
 *  Returns 0 on success. */
ULTRAFAST_SECP256K1_API int ultrafast_secp256k1_bip32_derive_path(
...
ULTRAFAST_SECP256K1_API int ultrafast_secp256k1_bip32_get_privkey(
    const secp256k1_bip32_key* key,
    uint8_t privkey_out[32]);

/** Get compressed public key from extended key.
 *  Returns 0 on success. */
ULTRAFAST_SECP256K1_API int ultrafast_secp256k1_bip32_get_pubkey(
    const secp256k1_bip32_key* key,
    uint8_t pubkey_out[33]);
 ...
 *  Returns 0 on success. */
ULTRAFAST_SECP256K1_API int ultrafast_secp256k1_taproot_output_key(
    const uint8_t internal_key_x[32],
 ...
 *  Returns 0 on success. */
ULTRAFAST_SECP256K1_API int ultrafast_secp256k1_taproot_tweak_privkey(
    const uint8_t privkey[32],
    const uint8_t* merkle_root,
 ...
 *  Returns 1 if valid, 0 if invalid. */
ULTRAFAST_SECP256K1_API int ultrafast_secp256k1_taproot_verify_commitment(
