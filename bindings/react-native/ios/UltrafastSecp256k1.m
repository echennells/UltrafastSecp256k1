...
- (instancetype)init {
    self = [super init];
    if (self) { ultrafast_secp256k1_init(); }
    return self;
}

RCT_EXPORT_BLOCKING_SYNCHRONOUS_METHOD(version) {
    return [NSString stringWithUTF8String:ultrafast_secp256k1_version()];
}
...
RCT_EXPORT_METHOD(ecPubkeyCreate:(NSString *)privkeyHex
    ...
    uint8_t out[33];
    if (ultrafast_secp256k1_ec_pubkey_create(pk.bytes, out) != 0) {
...
RCT_EXPORT_METHOD(ecPubkeyCreateUncompressed:(NSString *)privkeyHex
    ...
    NSData *pk = hexToData(privkeyHex);
    uint8_t out[65];
    if (ultrafast_secp256k1_ec_pubkey_create_uncompressed(pk.bytes, out) != 0) {
        reject(@"ERR", @"Invalid private key", nil);
        return;
    }
...
RCT_EXPORT_METHOD(ecPubkeyParse:(NSString *)pubkeyHex
    ...
    uint8_t out[33];
    if (ultrafast_secp256k1_ec_pubkey_parse(inp.bytes, inp.length, out) != 0) {
...
RCT_EXPORT_METHOD(ecSeckeyVerify:(NSString *)privkeyHex
                  ...
                  reject:(RCTPromiseRejectBlock)reject) {
    NSData *pk = hexToData(privkeyHex);
    resolve(@(ultrafast_secp256k1_ec_seckey_verify(pk.bytes) == 1));
}
...
RCT_EXPORT_METHOD(ecPrivkeyNegate:(NSString *)privkeyHex
    ...
    ultrafast_secp256k1_ec_privkey_negate(pk.mutableBytes);
...
RCT_EXPORT_METHOD(ecPrivkeyTweakAdd:(NSString *)privkeyHex
    ...
    if (ultrafast_secp256k1_ec_privkey_tweak_add(pk.mutableBytes, tw.bytes) != 0) {
...
RCT_EXPORT_METHOD(ecPrivkeyTweakMul:(NSString *)privkeyHex
    ...
    NSMutableData *pk = [hexToData(privkeyHex) mutableCopy];
    NSData *tw = hexToData(tweakHex);
    if (ultrafast_secp256k1_ec_privkey_tweak_mul(pk.mutableBytes, tw.bytes) != 0) {
...
RCT_EXPORT_METHOD(ecdsaSign:(NSString *)msgHashHex
    ...
    NSData *pk = hexToData(privkeyHex);
    uint8_t sig[64];
    if (ultrafast_secp256k1_ecdsa_sign(mh.bytes, pk.bytes, sig) != 0) {
...
RCT_EXPORT_METHOD(ecdsaVerify:(NSString *)msgHashHex
    ...
    resolve(@(ultrafast_secp256k1_ecdsa_verify(mh.bytes, s.bytes, pk.bytes) == 1));
}
...
RCT_EXPORT_METHOD(ecdsaSerializeDer:(NSString *)sigHex
    ...
    uint8_t der[72];
    size_t der_len = 72;
    if (ultrafast_secp256k1_ecdsa_signature_serialize_der(s.bytes, der, &der_len) != 0) {
...
RCT_EXPORT_METHOD(ecdsaSignRecoverable:(NSString *)msgHashHex
    ...
    uint8_t sig[64];
    int recid;
    if (ultrafast_secp256k1_ecdsa_sign_recoverable(mh.bytes, pk.bytes, sig, &recid) != 0) {
...
RCT_EXPORT_METHOD(ecdsaRecover:(NSString *)msgHashHex
    ...
    NSData *s = hexToData(sigHex);
    uint8_t pubkey[33];
    if (ultrafast_secp256k1_ecdsa_recover(mh.bytes, s.bytes, recid, pubkey) != 0) {
...
RCT_EXPORT_METHOD(schnorrSign:(NSString *)msgHex
    ...
    uint8_t sig[64];
    if (ultrafast_secp256k1_schnorr_sign(m.bytes, pk.bytes, ar.bytes, sig) != 0) {
...
RCT_EXPORT_METHOD(schnorrVerify:(NSString *)msgHex
    ...
    resolve(@(ultrafast_secp256k1_schnorr_verify(m.bytes, s.bytes, px.bytes) == 1));
}
...
RCT_EXPORT_METHOD(schnorrPubkey:(NSString *)privkeyHex
    ...
    NSData *pk = hexToData(privkeyHex);
    uint8_t out[32];
    if (ultrafast_secp256k1_schnorr_pubkey(pk.bytes, out) != 0) {
        reject(@"ERR", @"Invalid private key", nil);
        return;
    }
...
RCT_EXPORT_METHOD(ecdh:(NSString *)privkeyHex
    ...
    uint8_t out[32];
    if (ultrafast_secp256k1_ecdh(pk.bytes, pub.bytes, out) != 0) {
...
RCT_EXPORT_METHOD(ecdhXonly:(NSString *)privkeyHex
    ...
    NSData *pub = hexToData(pubkeyHex);
    uint8_t out[32];
    if (ultrafast_secp256k1_ecdh_xonly(pk.bytes, pub.bytes, out) != 0) {
...
RCT_EXPORT_METHOD(ecdhRaw:(NSString *)privkeyHex
    ...
    NSData *pub = hexToData(pubkeyHex);
    uint8_t out[32];
    if (ultrafast_secp256k1_ecdh_raw(pk.bytes, pub.bytes, out) != 0) {
...
RCT_EXPORT_METHOD(sha256:(NSString *)dataHex
    ...
    uint8_t out[32];
    ultrafast_secp256k1_sha256(d.bytes, d.length, out);
    resolve(dataToHex(out, 32));
}
...
RCT_EXPORT_METHOD(hash160:(NSString *)dataHex
    ...
    NSData *d = hexToData(dataHex);
    uint8_t out[20];
    ultrafast_secp256k1_hash160(d.bytes, d.length, out);
...
RCT_EXPORT_METHOD(taggedHash:(NSString *)tag
    ...
    NSData *d = hexToData(dataHex);
    uint8_t out[32];
    ultrafast_secp256k1_tagged_hash(tag.UTF8String, d.bytes, d.length, out);
    resolve(dataToHex(out, 32));
}
...
RCT_EXPORT_METHOD(addressP2PKH:(NSString *)pubkeyHex
    ...
    if (ultrafast_secp256k1_address_p2pkh(pk.bytes, network, addr, &alen) != 0) {
...
RCT_EXPORT_METHOD(addressP2WPKH:(NSString *)pubkeyHex
    ...
    NSData *pk = hexToData(pubkeyHex);
    char addr[128]; size_t alen = 128;
    if (ultrafast_secp256k1_address_p2wpkh(pk.bytes, network, addr, &alen) != 0) {
...
RCT_EXPORT_METHOD(addressP2TR:(NSString *)internalKeyXHex
    ...
    char addr[128]; size_t alen = 128;
    if (ultrafast_secp256k1_address_p2tr(ik.bytes, network, addr, &alen) != 0) {
...
RCT_EXPORT_METHOD(wifEncode:(NSString *)privkeyHex
    ...
    if (ultrafast_secp256k1_wif_encode(pk.bytes, compressed ? 1 : 0, network, wif, &wlen) != 0) {
...
RCT_EXPORT_METHOD(wifDecode:(NSString *)wif
    ...
    if (ultrafast_secp256k1_wif_decode(wif.UTF8String, pk, &comp, &net) != 0) {
...
RCT_EXPORT_METHOD(bip32MasterKey:(NSString *)seedHex
    ...
    if (ultrafast_secp256k1_bip32_master_key(seed.bytes, seed.length, &key) != 0) {
...
RCT_EXPORT_METHOD(bip32DerivePath:(NSString *)masterKeyHex
    ...
    if (ultrafast_secp256k1_bip32_derive_path(&master, path.UTF8String, &out) != 0) {
...
RCT_EXPORT_METHOD(taprootOutputKey:(NSString *)internalKeyXHex
    ...
    if (ultrafast_secp256k1_taproot_output_key(ik.bytes, mr ? mr.bytes : NULL, out, &parity) != 0) {
...
RCT_EXPORT_METHOD(taprootTweakPrivkey:(NSString *)privkeyHex
    ...
    NSData *mr = merkleRootHex ? hexToData(merkleRootHex) : nil;
    uint8_t out[32];
    if (ultrafast_secp256k1_taproot_tweak_privkey(pk.bytes, mr ? mr.bytes : NULL, out) != 0) {
...
RCT_EXPORT_METHOD(taprootVerifyCommitment:(NSString *)outputKeyXHex
    ...
    NSData *ik = hexToData(internalKeyXHex);
    NSData *mr = merkleRootHex ? hexToData(merkleRootHex) : nil;
    int rc = ultrafast_secp256k1_taproot_verify_commitment(
