...
static bool run_external_vectors(bool verbose) {
    ...
    // Suppression rationale for cppsecurity:S2083, marker on the ifstream line
    // below -- a marker alone on a comment line suppresses nothing.
    // The path comes from SECP256K1_SELFTEST_VECTORS,
    ...
    std::ifstream in(path);  // NOSONAR cppsecurity:S2083 -- see the block above
    if (!in) {
        if (verbose) {
            // Rationale for cppsecurity:S5145 (marker on the call below):
            // `path` is operator-supplied, and this is a console diagnostic,
            // not a structured log another system parses.
