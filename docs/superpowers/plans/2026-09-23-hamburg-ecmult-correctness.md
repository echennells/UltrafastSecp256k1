...
Proceed only after Task 0 RED. The independent algebra audit found that **only the second addition in final group 0** can reach `S1+S2=0`; the comment claiming universal nondegeneracy is false. The proof uses signed odd digits `d=2w−31`, the GLV lattice basis with determinant n, and the three exceptional relations `A=−B`, `A=−φ(B)`, `A=−φ²(B)`. Earlier groups `j≥1` have kernel-vector coordinate bound `2^125+30`, smaller than the shortest nonzero lattice representation; parity/top-digit signs exclude the zero vector. For final-group first add, the three candidate vectors are `(32U1+d1,32U2)`, `(32U1,32U2+d1)`, `(32U1−d1,32U2−d1)`; inverse-basis bounds constrain lattice coefficients to `m∈[−5,5]`, `r∈[−4,4]`, and their residues mod 32 rule out every required odd pattern. Final-group second add has the concrete witness. Preserve `HAMBURG=true` for the initial add and every group `j>0`; peel group 0 into fixed code and call the generic constant-time `unified_add_core<false,false>` **only** for its second addition. No scalar-dependent branch. Write/extend tests for the witness, zero, `n−1`, and deterministic random pairs before implementation. Freeze a same-flags pre-fix C++ `ecmult_const_xonly`/ElligatorSwift XDH benchmark binary before the production edit; never rebuild it afterward. Then measure; no release acceptance on correctness alone if speed regresses reproducibly.
