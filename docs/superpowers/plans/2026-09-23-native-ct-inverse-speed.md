...
2. Add a deterministic C++ gate for zero, one, `p−1`, limb edges, and at least 1,024 seeded valid inputs. Assert exact 32-byte canonical result against `fast::FieldElement52::inverse_safegcd()` where applicable, and `a * inv(a) == 1` for every nonzero input. Include stable aggregate checksum to compare pre/post binaries. First observe test failure by an intentionally wrong expected byte in a temporary test-only line, then correct it before green; never leave the deliberate failure in a commit.
